...
# --- plotting with EllipseModel (0°–360°) ---
def plot_ellipse_fits(chi, q_vs_chi, output_path=None, dpi=600):
    """
    Plot raw q_vs_chi data and overlay an ellipse fit for each ring,
    applying the azimuthal rotation from the ellipse model.
    """
    ...
    for j, ax in enumerate(axes):
        ...
        chi_fit = np.rad2deg(t + theta) % 360
        sort_fit = np.argsort(chi_fit)
        chi_fit_sorted = chi_fit[sort_fit]
        rt_sorted = rt[sort_fit]
        ax.plot(chi_plot, r_vals, '.', ms=2, label='data')
        ax.plot(chi_fit_sorted, rt_sorted, '-', lw=1, label='ellipse fit')
...
def main():
    ...
    q_min_nm1   = 16.0
    npt_rad     = 5000
    output_png  = f"OutputFiles/QvsAz/SSAO-6_25C_Map_ellipse_{num_azim}bin.png"

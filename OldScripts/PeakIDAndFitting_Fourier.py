...
# --- new plotting with Fourier fits ---
def plot_fitted_q_vs_chi(chi, q_vs_chi, delta_tol, output_path=None, dpi=600):
    """
    Plot raw q_vs_chi and overlay Fourier fit for each ring in subplots.
    """
    ...
    for j, ax in enumerate(axes):
        ...
        if np.all(np.isnan(r_vals)):
            continue
        coeffs, r_fit = fit_fourier(r_vals, phi)
        ax.plot(chi, r_vals, '.', ms=2, label='data')
...
def main():
    ...
    npt_rad         = 5000
    delta_tol       = 0.05
    output_png      = f"OutputFiles/Fits/SSAO-6_25C_Map_100x100_fourier_{num_azim_bins}bin_r1.png"
    ...
    # detect and fit peaks (to get center indices)
    q_peaks, pw, wq = detect_global_peaks(I2d, q)
    # fit each slice pseudo-Voigt to get q_vs_chi
    q_vs_chi       = fit_slices_parallel(I2d, q_peaks, pw, wq, q)
    # plot Fourier fits
    plot_fitted_q_vs_chi(chi, q_vs_chi, delta_tol, output_path=output_png)

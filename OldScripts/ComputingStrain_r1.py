...
def load_integrator_and_data(poni_path, tif_path, ref_tif_path=None, mask_threshold=4e2):
    """
    Load PyFAI integrator and adjust a 2D XRD image to match the brightness/contrast
    of a reference TIF (if provided), otherwise do a percentile stretch.
    ...
    # 2) Contrast adjustment using ImageJ-style autocontrast
    data_adj_float = imagej_autocontrast(data, k=2.5)
    ...
    # 3) Convert back to original dtype
    orig_dtype = img.data.dtype
    if np.issubdtype(orig_dtype, np.integer):
        info = np.iinfo(orig_dtype)
        data_adj_float = np.clip(data_adj_float, info.min, info.max)
    data_adj = data_adj_float.astype(orig_dtype)
    ...
    # 5) Build mask on the adjusted data
    mask = data_adj > mask_threshold

    return ai, data_adj, mask
...
def _fit_slice(intensity_row, q_peaks, peak_windows, widths_q, q, delta_tol=0.05, eta0=0.5):
    out = []
    for q0, wid0 in zip(q_peaks, widths_q):
        ...
        try:
            ...
            amp_thresh = 0.005 * np.max(intensity_row)
            max_width = 2.0  # max allowed FWHM in q-units
            if popt[0] < amp_thresh or popt[2] > max_width:
                out.append(np.nan)
            else:
                out.append(popt[1])
...
def fit_fourier(r_vals, phi_vals, harmonics):
    if len(r_vals) != len(phi_vals):
        print("Warning: Fourier fit skipped due to length mismatch.")
        return np.full_like(r_vals, np.nan), np.full_like(r_vals, np.nan)
...
def main():
    ...
    ai, data, mask = load_integrator_and_data(
        poni_file,
        tif_file,

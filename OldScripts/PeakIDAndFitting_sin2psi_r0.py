...
def load_integrator_and_data(poni_path, tif_path, mask_threshold=0):
...
# --- Sin2Psi fit helper -------------------------------------------------
def fit_sin2psi(r_vals, psi_vals):
    x = np.sin(psi_vals) ** 2
    mask = ~np.isnan(r_vals)
    coeffs = np.polyfit(x[mask], r_vals[mask], 1)
    r_fit = np.polyval(coeffs, x)
    return coeffs, r_fit

# --- Q vs Azimuth stacked plot ------------------------------------------
def plot_fitted_q_vs_chi(chi, q_vs_chi, delta_tol,
    ...
    for j, ax in enumerate(axes):
        ...
        if np.all(np.isnan(r_vals)):
            continue
        _, r_fit = fit_sin2psi(r_vals, phi)
        ax.plot(chi, r_vals, '.', ms=1, label='Peak centroids')
        ax.plot(chi, r_fit, '-', lw=1, label='Sin2Psi fit')
        m = np.nanmean(r_vals)
        ax.set_ylim(m - delta_tol, m + delta_tol)
        ax.set_xlim(0, 360)
        plt.xticks(np.arange(0, 360+10, 30))
...
def plot_radar_all_rings(chi, q_vs_chi, output_path=None, dpi=600):
    phi = np.deg2rad(chi)
...
def main():
    ...
    tif_file       = "InputFiles/VB-APS-SSAO-6_25C_Map-AO_000509.avg_adjusted.tif"
    mask_thresh    = 4e9
    num_azim_bins  = 120
    q_min_nm1      = 16.0
    npt_rad        = 5000
    delta_tol      = 0.05
    eta0           = 0.5
    qazi_png       = f"OutputFiles/Fits/SSAO-6_25C_509_100x100_sin2psi_{num_azim_bins}bin.png"
    radar_png      = f"OutputFiles/Fits/SSAO-6_25C_509_100x100_radar_{num_azim_bins}bin.png"
    ...
    # shift chi to [0,360) and reorder
    chi360 = (chi + 360) % 360
    ...
    q_vs_chi       = fit_slices_parallel(I2d, q_peaks, pw, wq, q,
                                         delta_tol=delta_tol, eta0=eta0)
    # reorder centroid data to match 0–360 chi
    q_vs_chi = q_vs_chi[:, order]

    plot_fitted_q_vs_chi(chi360, q_vs_chi, delta_tol,
                         output_path=qazi_png, dpi=600)
    # plot_radar_all_rings(chi360, q_vs_chi,

...
# --- Fourier fit helper -------------------------------------------------
def fit_fourier(r_vals, phi_vals, harmonics=(1,2)):
    """
    Fit r(phi) = a0 + sum_{n in harmonics}[a_n cos(n phi) + b_n sin(n phi)].
    Returns coeffs and fitted curve.
    """
    # build design matrix
...
def integrate_2d(ai, data, mask, num_azim_bins=360, q_min=16.0, npt_rad=5000):
    # full radial axis for determining q_max
    ...
    # ensure shape = [n_azim, n_rad]
    if I2d.shape == (len(q), len(chi)):
        I2d = I2d.T
    return I2d, q, chi

# --- Global peak detection & windowing ----------------------------------
...
# --- Slice‑by‑slice Pseudo‑Voigt fitting -------------------------------
...
def fit_slices_parallel(I2d, q_peaks, peak_windows, widths_q, q,
    ...
    # transpose so shape = [n_rings, n_chi]
    return np.array(results).T

# --- Plotting with Fourier overlay --------------------------------------
def plot_fitted_q_vs_chi(chi, q_vs_chi, delta_tol, harmonics=(1,2),
                         output_path=None, dpi=600):
    """
    For each ring, plot the Pseudo‑Voigt‑extracted centroid q_vs_chi[j]
    and overlay the Fourier fit of r(phi).
    """
    n_rings, n_chi = q_vs_chi.shape
    phi = np.deg2rad(chi)
    ...
    for j, ax in enumerate(axes):
        ...
        # perform Fourier fit on the Pseudo‑Voigt centroids
        coeffs, r_fit = fit_fourier(r_vals, phi, harmonics)
...
def main():
    ...
    npt_rad       = 5000
    delta_tol     = 0.05
    output_png    = f"OutputFiles/Fits/SSAO_fourier_{num_azim_bins}bin.png"

    # run pipeline
    ai, data, mask = load_integrator_and_data(poni_file, tif_file, mask_thresh)
    I2d, q, chi    = integrate_2d(ai, data, mask, num_azim_bins, q_min_nm1, npt_rad)
    q_peaks, pw, wq = detect_global_peaks(I2d, q)
    q_vs_chi       = fit_slices_parallel(I2d, q_peaks, pw, wq, q)
    plot_fitted_q_vs_chi(chi, q_vs_chi, delta_tol, harmonics=(1,2),
                         output_path=output_png, dpi=600)
